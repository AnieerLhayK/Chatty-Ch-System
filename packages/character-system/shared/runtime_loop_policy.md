...
When `style-doctor` is operating in `record_write` mode, diagnosis and handoff
packets must be saved as Markdown files in the default runtime-loop directories.
They should not remain only as chat transcript text. If a platform cannot save
...
- keep it short, for example `target-character-case008-social-critique`;
- if no case exists, use the task type, for example `target-character-runtime-rewrite`;
...
`session_snapshot` should summarize the minimum interaction context needed to
reassess the diagnosis. `session_storage` should point to the platform session,
thread id, transcript export, saved local conversation file, or state
...
## Protecting target character-Specific Lessons

target character is a mature character-specific artifact. Its runtime lessons should default to `character-specific` unless the maintainer explicitly records a generalization candidate with evidence that the lesson applies beyond target character.

Never treat target character's special structure, pacing, emotional restraint, imagery limits, or handcrafted prompt choices as generator defaults from one patch. Use the generalization backlog for review instead.
